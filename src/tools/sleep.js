module.exports = {
    getAverageTime: function(dates){
        return dates.map((d) => module.exports.getHoursFromUTCMidnight(d)).reduce((p, a) => p + a) / dates.length;
    },
    ...
    getBedTimeSessions: function (sessions){
        ...
        for(let i = 1; i < completedSessions.length - 1; i++){
            ...
            if(hoursUntilNext > 5 && hoursUntilNext < 12 && hoursSincePrevious < 12){
    ...
    getAnalyzedBedTimeSessions: function(sessions){
        ...
        if(bedTimeSessions.length < 1) return;
        ...
        const startDates = sortedSessions
            .filter((s, i) => {
                const last = sortedSessions[Math.min(sortedSessions.length - 1, i + 1)];
